...
- **Don't** assume the 13 document pages (blog.html, preco-site-suica.html, caso-hotel-alpina.html, the other articles and the legal pages) share this file: they carry the same Graphite tokens and Archivo type but their own smaller "document" stylesheet (self-contained per page), plus the shared `css/motion.css` + `js/motion.js` reveal layer (word-split `main h1`/`h2`, scroll reveal, `.cta` wipe) — the homepage's marquee, cursor dot and hero mechanics don't apply there.
