...
(async () => { const b = await chromium.launch({ executablePath: CHROME });
  ...
  for (const pg of pages) for (const w of [320, 430, 768, 1024, 1280, 1920]) {
    ...
    /* A home leva content-visibility nas secções, e com ele vem contain:paint.
       Isso corta o que transborda e tira-o do scrollWidth: o transbordo real
       deixava de aparecer aqui e o teste ficava verde para sempre, a esconder
       a avaria em vez de a apanhar. Desliga-se a contenção, deixa-se compor a
       página toda, e só depois se mede. */
